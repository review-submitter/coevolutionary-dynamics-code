...
class Parameters:
    ...
    def __init__(self):
        ...
        # non mutation condition:
        # self.prey_mutation_rate = 0  # mutation rate of prey per division
        # self.predator_mutation_rate = 0  # mutation rate of predation per division
...
def parse_arguments():
    ...
    parser.add_argument('std_dev', type=float, help='Standard deviation for prey and predator trait values')
...
def run_simulation(params,m, std_dev, ancestor_prey_growth,
    ...
    logging_info = (
        f"Simulation Begins! Predation scaling coefficient m={m}, "
        f"Standard deviation={std_dev}, Ancestor prey growth={ancestor_prey_growth}, "
        f"Ancestor predator capacity={ancestor_predator_capacity}"
    )
    logging.info(logging_info)
    
    # set up initial conditions
    prey_std_dev = std_dev * ancestor_prey_growth
    predator_std_dev = std_dev * ancestor_predator_capacity
    # set predator std_dev to very laege

    # Explore the initial conditions 
    # Before: ancestor_prey_num = 1000, ancestor_pred_num = 100
    ...
    while t <= max_time:
        ...
        if len(current_prey) == 0:
            ...
            if reaction_series_index == 0:
                ...
                current_prey[reaction_index].prey_mutation(cf.get_truncnorm_number, current_prey, current_prey[reaction_index].g_value, prey_std_dev, 0, 1.0)
                ...
                current_prey[reaction_index[0]].prey_die()
                current_predator[reaction_index[1]].predator_mutation(cf.get_truncnorm_number, current_predator, current_predator[reaction_index[1]].k_value, predator_std_dev, 0, 0.3)
                # set predator mutation variation very large
...
if __name__ == "__main__":
    ...
    output_filename = output_path + 'simulation_data_{}.pkl'.format(args[4])
    run_simulation(params, *args[:4], output_filename, args[5])
